...
class Pesanan():
    def __init__(self, id_pesanan, id_menu, kuantitas):
        ...
        try:
            ...
            with connection.cursor() as cursor:
                cursor.execute("INSERT INTO pesanan (id_pesanan, id_menu, kuantitas) VALUES (%s, %s, %s)", (
                    self.id_pesanan, self.id_menu, self.kuantitas,))
            connection.commit()
            connection.close()
    ...
    def getPesananById(cls, id_pesanan):
        try:
            ...
            with connection.cursor() as cursor:
                cursor.execute(
                    "SELECT * FROM pesanan WHERE id_pesanan = (%s)", (id_pesanan,))
                resultset = cursor.fetchall()
            connection.commit()
            connection.close()
    ...
    def getMenuPesananById(cls, id_pesanan, id_menu):
        try:
            ...
            with connection.cursor() as cursor:
                cursor.execute(
                    "SELECT * FROM pesanan WHERE id_pesanan = (%s) AND id_menu = (%s)", (id_pesanan, id_menu,))
                resultset = cursor.fetchall()
            connection.commit()
            connection.close()
    ...
    def deleteMenuInPesanan(self, id_pesanan, id_menu):
        try:
            ...
            with connection.cursor() as cursor:
                cursor.execute(
                    "DELETE FROM pesanan WHERE id_pesanan = (%s) AND id_menu = (%s)", (id_pesanan, id_menu,))
            connection.commit()
            connection.close()
    ...
    def deletePesanan(self, id_pesanan):
        try:
            ...
            with connection.cursor() as cursor:
                cursor.execute(
                    "DELETE FROM pesanan WHERE id_pesanan = (%s)", (id_pesanan,))
            connection.commit()
            connection.close()
